...
import { STAGE_ORDER, STAGE_SHORT_LABELS, STAGE_OWNER_ROLE, ROLE_LABELS, formatDate } from '@/lib/ecr-helpers'
...
export function StagePipeline({ currentStage, status, designInitialForm, stageHistories }: StagePipelineProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4">
      <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Stage Pipeline</h2>
      <div className="flex items-start gap-0">
        {STAGE_ORDER.map((stage, index) => {
          ...
          const isReturned = stageHistories.some(h => h.stage === stage && h.flow_status === 'RETURNED')
            ...
            <div key={stage} className="flex-1 flex items-start">
              ...
              <div className="flex-1">
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <p className={cn(
                        ...
                        isCurrent ? 'text-blue-700' : isCompleted ? 'text-green-700' : 'text-muted-foreground'
                      )}>
                        {STAGE_SHORT_LABELS[stage]}
                      </p>
                      <p className="text-[10px] text-muted-foreground/70 mt-0.5">{ROLE_LABELS[ownerRole]}</p>
                    </div>
                  ...
                  {/* Stage status line */}
                  {isSkipped && (
                    <p className="text-[10px] text-slate-400 italic">Skipped</p>
                  )}
                  {isCompleted && history && (
                    <div>
                      <p className="text-[10px] text-green-600">{formatDate(history.created_at)}</p>
                      {history.acted_by_name && (
                        <p className="text-[10px] text-muted-foreground truncate">{history.acted_by_name}</p>
                      )}
                      {history.remark && (
                        <p className="text-[10px] text-muted-foreground/70 italic truncate mt-0.5">&ldquo;{history.remark}&rdquo;</p>
                      )}
                    </div>
                  )}
                  {isCurrent && !isSkipped && (
        ...
        <div className={cn(
          ...
          <CheckCircle2 className={cn('w-4 h-4 mb-1', isReleased ? 'text-green-600' : 'text-slate-300')} />
          <p className={cn('text-[10px] font-semibold', isReleased ? 'text-green-700' : 'text-muted-foreground')}>
            Released
          </p>
        </div>
      </div>
