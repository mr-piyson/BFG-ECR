...
export function StageFormPanel(props: StageFormPanelProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg">
      ...
      <div className="p-4">
        {activeTab === 'stage1' && (
          <Stage1Form ecr={ecr} userId={userId} form={designInitialForm} onUpdate={onUpdate} />
        )}
        {activeTab === 'stage2' && (
          <Stage2Form
            ecr={ecr}
        ...
        {activeTab === 'stage4' && (
          <Stage4Form ecr={ecr} userId={userId} form={designMeetingForm} onUpdate={onUpdate} />
        )}
        {activeTab === 'stage5' && (
          <Stage5Form
            ecr={ecr}
...
function AuditTrail({
  ...
  histories: (StageHistory & { acted_by_name?: string; acted_by_role?: string })[];
}) {
        ...
        {[...histories].reverse().map((h) => {
          ...
          return (
            <div key={h.id} className="relative flex items-start gap-4 pl-10">
              <div
                className={cn(
                  'absolute left-3 top-1.5 w-2.5 h-2.5 rounded-full border-2 border-card',
                  dotColor[h.flow_status],
                )}
              />
              <div className="flex-1 bg-muted/30 border border-border rounded-lg p-3">
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {h.acted_by_role && (
                    <span className="ml-1">
                      ({ROLE_LABELS[h.acted_by_role as keyof typeof ROLE_LABELS] || h.acted_by_role}
                      )
                    </span>
                  )}
                </div>
                {h.returned_to_stage && (
                  <div className="mt-1 text-xs text-orange-600">
                    Returned to: {STAGE_SHORT_LABELS[h.returned_to_stage]}
                  </div>
                )}
                {h.remark && (
