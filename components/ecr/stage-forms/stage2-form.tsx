...
export function Stage2Form({ ecr, userId, form, designInitialForm, onUpdate }: Stage2FormProps) {
  ...
  const isActive = ecr.current_stage === "COSTING" && ["PENDING_COSTING", "UNDER_COSTING"].includes(ecr.status);
  ...
  const [data, setData] = useState({
    ...
    offer_to_customer_date: form?.offer_to_customer_date ? form.offer_to_customer_date.split("T")[0] : "",
  ...
  async function handleProcess() {
    ...
    try {
      const res = await fetch(`/api/ecrs/${ecr.id}/stage`, {
        ...
        body: JSON.stringify({ action: "process_costing", userId, stage: "COSTING", formData: data, remark }),
      });
  ...
  if (isSkipped) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="text-xs text-muted-foreground/70 mt-1">Design Engineer marked this stage as not applicable (no cost impact).</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Assess cost impact and fill quote details</p>
        </div>
        {isCompleted && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label="Costing Engineer" required>
          <input type="text" value={data.costing_engineer_id} onChange={(e) => setData((p) => ({ ...p, costing_engineer_id: e.target.value }))} disabled={!isActive} placeholder="Wafa Ali" className="form-input" />
        </FormField>
        <FormField label="Currency">
          <select value={data.currency} onChange={(e) => setData((p) => ({ ...p, currency: e.target.value }))} disabled={!isActive} className="form-input">
        ...
        <FormField label="Date of Quote">
          <input type="date" value={data.date_of_quote} onChange={(e) => setData((p) => ({ ...p, date_of_quote: e.target.value }))} disabled={!isActive} className="form-input" />
        </FormField>
        <FormField label="Offer to Customer Date">
          <input type="date" value={data.offer_to_customer_date} onChange={(e) => setData((p) => ({ ...p, offer_to_customer_date: e.target.value }))} disabled={!isActive} className="form-input" />
        </FormField>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 bg-muted/20 space-y-3">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={data.has_nrc_cost} onChange={(e) => setData((p) => ({ ...p, has_nrc_cost: e.target.checked }))} disabled={!isActive} className="rounded" />
          <span className="text-sm font-medium text-foreground">Non-Recurring Cost (NRC)</span>
        </label>
        {data.has_nrc_cost && (
          <FormField label="NRC Amount">
            <input type="number" step="0.01" value={data.nrc_amount} onChange={(e) => setData((p) => ({ ...p, nrc_amount: e.target.value }))} disabled={!isActive} placeholder="45000.00" className="form-input" />
          </FormField>
        )}
      </div>
      ...
      <div className="border border-border rounded-lg p-4 bg-muted/20 space-y-3">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={data.has_rc_cost} onChange={(e) => setData((p) => ({ ...p, has_rc_cost: e.target.checked }))} disabled={!isActive} className="rounded" />
          <span className="text-sm font-medium text-foreground">Recurring Cost (RC)</span>
        </label>
        {data.has_rc_cost && (
          <FormField label="RC Amount">
            <input type="number" step="0.01" value={data.rc_amount} onChange={(e) => setData((p) => ({ ...p, rc_amount: e.target.value }))} disabled={!isActive} placeholder="1200.00" className="form-input" />
          </FormField>
        )}
      </div>

      <FormField label="Cost Details / Notes">
        <textarea value={data.cost_details} onChange={(e) => setData((p) => ({ ...p, cost_details: e.target.value }))} disabled={!isActive} rows={3} className="form-input resize-none" placeholder="Additional cost notes..." />
      </FormField>
      ...
      {showReturn && <ReturnDialog ecrId={ecr.id} userId={userId} stage="COSTING" currentStatus={ecr.status} availableTargets={[{ value: "DESIGN_ENGINEER_INITIAL", label: "Stage 1 — Design Engineer" }]} onClose={() => setShowReturn(false)} onUpdate={onUpdate} />}
    </div>
  );
}
