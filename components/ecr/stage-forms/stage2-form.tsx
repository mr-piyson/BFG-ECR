    ...
    <div className="space-y-5">
      ...
      {isCompleted && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-800 space-y-1">
          {data.has_nrc_cost && <p>NRC: <strong>{formatCurrency(data.nrc_amount, data.currency)}</strong></p>}
          {data.has_rc_cost && <p>RC: <strong>{formatCurrency(data.rc_amount, data.currency)}</strong></p>}
      ...
      {showReturn && (
        <ReturnDialog
          ...
          availableTargets={[{ value: 'DESIGN_ENGINEER_INITIAL', label: 'Stage 1 — Design Engineer' }]}
