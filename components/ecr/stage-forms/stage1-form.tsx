...
export function Stage1Form({ ecr, userId, form, onUpdate }: Stage1FormProps) {
  ...
  if (!form) {
    return (
      <div className="text-sm text-muted-foreground text-center py-8">No Stage 1 form found.</div>
    );
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">
            Stage 1 — Design Engineer: Initial CR Form
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Fill the customer change request details and submit to workflow
          </p>
        </div>
      ...
      <div className="border border-border rounded-lg p-4 bg-muted/20">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Check to skip stages that are not applicable to this ECR.
        </p>
...
function SkipToggle({
  ...
  return (
    <label
      className={`flex items-start gap-2 p-3 rounded-lg border cursor-pointer transition-colors ${checked ? 'border-orange-200 bg-orange-50' : 'border-border bg-card hover:bg-muted/30'} ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}
    >
      <input
        type="checkbox"
        checked={checked}
