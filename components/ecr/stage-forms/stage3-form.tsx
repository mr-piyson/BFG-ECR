...
export function Stage3Form({ ecr, userId, form, designInitialForm, onUpdate }: Stage3FormProps) {
  ...
  const isActive =
    ecr.current_stage === 'PROJECT_MANAGER' &&
    ['PENDING_PROJECT_MANAGER', 'UNDER_PROJECT_MANAGER'].includes(ecr.status);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Fill PO receipt details and Rank of Application
          </p>
        </div>
        {isCompleted && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label="PO Receipt Date">
          <input
            type="date"
            value={data.po_receipt_date}
            onChange={(e) => setData((p) => ({ ...p, po_receipt_date: e.target.value }))}
            disabled={!isActive}
        ...
        <FormField label="RoA — Rank of Application" hint="Train set number(s)">
          <input
            type="text"
            value={data.roa}
            onChange={(e) => setData((p) => ({ ...p, roa: e.target.value }))}
            disabled={!isActive}
            placeholder="Train Set: TGV-001, TGV-002"
            className="form-input"
          />
        </FormField>
      </div>
      ...
      <FormField label="PM Notes">
        <textarea
          value={data.pm_notes}
          onChange={(e) => setData((p) => ({ ...p, pm_notes: e.target.value }))}
          disabled={!isActive}
          rows={4}
          className="form-input resize-none"
          placeholder="PO confirmed. Notes on application scope..."
        />
      </FormField>
  ...
  return (
    <div className="space-y-1">
      <label className="text-xs font-medium text-foreground">
        {label}
        {hint && (
          <span className="text-muted-foreground ml-1 font-normal text-[10px]">({hint})</span>
        )}
      </label>
      {children}
    </div>
