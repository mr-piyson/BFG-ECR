...
interface Stage4FormProps {
  ...
  form:
    | (DesignMeetingForm & {
        attendees?: { id: string; name: string; email: string | null; is_external: boolean }[];
      })
    | null;
  onUpdate: () => void;
}
...
export function Stage4Form({ ecr, userId, form, onUpdate }: Stage4FormProps) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">
            Stage 4 — Design Engineer: Meeting & Release
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Schedule the ECR release meeting and fill release dates
          </p>
        </div>
        {isCompleted && (
      ...
      <label
        className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${data.is_not_applicable ? 'border-slate-300 bg-slate-50' : 'border-border hover:bg-muted/30'}`}
      >
        <input
          type="checkbox"
          checked={data.is_not_applicable}
          onChange={(e) => setData((p) => ({ ...p, is_not_applicable: e.target.checked }))}
          disabled={!isActive}
          className="rounded"
        />
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            e.g. paint-scope-only changes — no meeting required
          </p>
        </div>
      </label>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField label="Meeting Date">
              <input
                ...
                onChange={(e) => setData((p) => ({ ...p, meeting_date: e.target.value }))}
                disabled={!isActive}
            ...
            <FormField label="EPICOR Release Date">
              <input
                type="date"
                value={data.epicor_release_date}
                onChange={(e) => setData((p) => ({ ...p, epicor_release_date: e.target.value }))}
                disabled={!isActive}
                className="form-input"
              />
            </FormField>
            <FormField label="ERN Release Date">
              <input
                type="date"
                value={data.ern_release_date}
                onChange={(e) => setData((p) => ({ ...p, ern_release_date: e.target.value }))}
                disabled={!isActive}
                className="form-input"
              />
            </FormField>
          ...
          <FormField label="Meeting Notes">
            <textarea
              ...
              onChange={(e) => setData((p) => ({ ...p, meeting_notes: e.target.value }))}
              disabled={!isActive}
              rows={3}
                ...
                {attendees.map((att, i) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      {att.is_external ? 'External' : 'Internal'}
                    </span>
                    {isActive && (
                      <button
            ...
            {isActive && (
              <div className="flex items-center gap-2 p-3 bg-muted/20 rounded border border-dashed border-border">
                ...
                <label className="flex items-center gap-1.5 text-xs text-muted-foreground whitespace-nowrap cursor-pointer">
                  <input
                    type="checkbox"
                    checked={newAttendee.is_external}
                    onChange={(e) =>
                      setNewAttendee((p) => ({ ...p, is_external: e.target.checked }))
                    }
                    className="rounded"
                  />
                  External
