...
export function Stage5Form({ ecr, userId, form, designInitialForm, onUpdate }: Stage5FormProps) {
  ...
  const isActive =
    ecr.current_stage === 'QUALITY_FINAL_CHECK' &&
    ['PENDING_QUALITY_CHECK', 'UNDER_QUALITY_CHECK'].includes(ecr.status);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="text-xs text-muted-foreground/70 mt-1">
          Design Engineer marked this stage as not applicable.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Released on {formatDate(ecr.released_at)}
        </p>
        {form && (
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">
            Stage 5 — Quality Engineer: Final Check
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Verify the change has been correctly implemented in the train set
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label="Quality Engineer">
          <input
            type="text"
            value={data.quality_engineer_id}
            onChange={(e) => setData((p) => ({ ...p, quality_engineer_id: e.target.value }))}
            disabled={!isActive}
            placeholder="Diana Chen"
            ...
            {['OK', 'Not OK'].map((v) => (
              ...
              >
                <input
                  ...
                  value={v}
                  checked={data.verification_result === v}
                  onChange={() => setData((p) => ({ ...p, verification_result: v }))}
                  disabled={!isActive}
                  className="sr-only"
                />
        ...
        <FormField label="Verified In Train Set">
          <input
            type="text"
            value={data.verified_in_train_set}
            onChange={(e) => setData((p) => ({ ...p, verified_in_train_set: e.target.value }))}
            disabled={!isActive}
            placeholder="TGV-001"
            className="form-input"
          />
        ...
        <FormField label="Verification Date">
          <input
            ...
            onChange={(e) => setData((p) => ({ ...p, verification_date: e.target.value }))}
            disabled={!isActive}
            className="form-input"
          />
        </FormField>
      ...
      <FormField label="Findings">
        <textarea
          value={data.findings}
          onChange={(e) => setData((p) => ({ ...p, findings: e.target.value }))}
          disabled={!isActive}
          rows={4}
          className="form-input resize-none"
          placeholder="Describe the verification findings and any issues observed..."
        />
      </FormField>
        ...
        <>
          <FormField label="Remark (optional)">
            <textarea
              ...
              rows={2}
              className="form-input resize-none"
              placeholder="Optional remark when releasing..."
            />
          </FormField>
          <div className="flex items-center justify-between pt-2 border-t border-border">
            ...
            <button
              ...
              disabled={processing}
              className="inline-flex items-center gap-2 px-4 py-1.5 text-sm bg-green-600 text-white rounded hover:bg-green-700 transition-colors disabled:opacity-50 font-semibold"
            >
              {processing ? (
