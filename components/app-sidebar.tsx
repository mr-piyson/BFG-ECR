...
import { LayoutDashboard, FileText, Settings, ChevronRight, Activity } from 'lucide-react';
  ...
  return (
    <aside className="w-56 flex-shrink-0 bg-sidebar text-sidebar-foreground flex flex-col border-r border-sidebar-border">
      ...
      <div className="px-3 py-3 border-t border-sidebar-border">
        <p className="text-[10px] uppercase tracking-wider text-sidebar-foreground/40 mb-2 px-1">
          Active User
        </p>
        <UserSwitcher />
      </div>
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute bottom-full left-0 right-0 mb-1 bg-sidebar border border-sidebar-border rounded shadow-lg hidden group-hover:block z-50">
        <p className="text-[10px] uppercase tracking-wider text-sidebar-foreground/40 px-3 pt-2 pb-1">
          Switch User
        </p>
        {USERS.map((user) => (
          <button
          ...
          >
            <div className="w-5 h-5 rounded-full bg-sidebar-primary/60 flex items-center justify-center flex-shrink-0">
              <span className="text-[8px] font-bold text-sidebar-primary-foreground">
                {user.initials}
              </span>
            </div>
            <div>
