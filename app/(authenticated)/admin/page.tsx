...
export default function AdminPage() {
  ...
  const deleteScope = async (scopeId: string) => {
    if (!selectedProject) return;
    try {
      const res = await fetch(`/api/admin/projects/${selectedProject.id}/scopes/${scopeId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const deleteAssignment = async (assignmentId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/projects/${selectedProject.id}/assignments/${assignmentId}`,
        { method: 'DELETE' },
      );
      if (res.ok) {
    ...
    <div className="p-8 space-y-6">
                  ...
                  <tbody className="divide-y divide-border">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${user.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                          >
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  user will persist but the user account will be removed.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                  ...
                  <tbody className="divide-y divide-border">
                    {projects.map((project) => (
                      <tr key={project.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${project.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                          >
                        ...
                        <td className="px-6 py-4 text-right flex justify-end gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => openProjectDialog('edit', project)}
                          >
                            <Edit2 className="w-4 h-4" />
                          </Button>
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Removing a project will affect all associated ECRs. Consider
      ...
      <Dialog open={isUserDialogOpen} onOpenChange={setIsUserDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {userDialogMode === 'create' ? 'Add System User' : 'Update User Profile'}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="u-pass">
                {userDialogMode === 'edit' ? 'New Password (Optional)' : 'Password'}
              </Label>
              <Input
                id="u-pass"
                type="password"
            ...
            <div className="grid gap-2">
              ...
              >
                {Object.entries(USER_ROLES).map(([val, lbl]) => (
            ...
            <div className="flex items-center gap-2 pt-2">
              <Switch
                ...
                onCheckedChange={(c) => setUserForm((p) => ({ ...p, isActive: !!c }))}
              />
      ...
      <Dialog open={isProjectDialogOpen} onOpenChange={setIsProjectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {projectDialogMode === 'create' ? 'Define New Project' : 'Update Project Details'}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
            ...
            <div className="flex items-center gap-2 pt-2">
              <Switch
                ...
                onCheckedChange={(c) => setProjectForm((p) => ({ ...p, isActive: !!c }))}
              />
      ...
      <Dialog open={isProjectDetailOpen} onOpenChange={setIsProjectDetailOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-border pb-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setActiveScope(null);
                    setScopeForm({ name: '', description: '', isActive: true });
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {projectScopes.length === 0 ? (
                  <div className="col-span-2 text-center py-6 text-muted-foreground bg-muted/20 rounded-lg border border-dashed border-border">
                  ...
                  projectScopes.map((scope) => (
                    <Card key={scope.id} className="p-4 bg-muted/30 border-border relative group">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${scope.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                        >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-border pb-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    setAssignmentForm({ userId: '', role: 'DESIGN_ENGINEER', isPrimary: false });
                      ...
                      projectAssignments.map((asgn) => (
                        <tr key={asgn.id} className="hover:bg-muted/20">
                          ...
                          <td className="px-4 py-3 text-center">
                            {asgn.isPrimary && (
                              <span className="px-2.5 py-1 rounded-full text-[10px] bg-amber-100 text-amber-700 font-bold border border-amber-200 uppercase">
                                Primary Lead
                              </span>
                            )}
                          </td>
          ...
          <DialogFooter className="bg-muted/30 -mx-6 -mb-6 p-6 mt-10 border-t border-border">
            <Button onClick={() => setIsProjectDetailOpen(false)} className="px-8">
              Close Project View
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isScopeDialogOpen} onOpenChange={setIsScopeDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              <Switch
                checked={scopeForm.isActive}
                onCheckedChange={(c) => setScopeForm((p) => ({ ...p, isActive: !!c }))}
              />
          ...
          <DialogFooter>
            ...
            <Button onClick={saveScope} disabled={saving}>
              {saving ? 'Processing...' : 'Commit Scope'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="grid gap-6 py-4">
            <div className="grid gap-2">
              ...
              <select
                className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-primary"
                value={assignmentForm.userId}
                onChange={(e) => setAssignmentForm((p) => ({ ...p, userId: e.target.value }))}
              >
            ...
            <div className="grid gap-2">
              ...
              <select
                ...
                onChange={(e) => setAssignmentForm((p) => ({ ...p, role: e.target.value as any }))}
              >
                {Object.entries(USER_ROLES).map(([val, lbl]) => (
                  <option key={val} value={val}>
                    {lbl}
            ...
            <div className="flex items-center gap-3 p-4 bg-muted/40 rounded-lg border border-border">
              <Switch
                ...
                onCheckedChange={(c) => setAssignmentForm((p) => ({ ...p, isPrimary: !!c }))}
              />
              <div>
                <Label className="block mb-0.5">Primary Lead Designation</Label>
                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">
                  Sets as the main contact for this role
                </p>
              </div>
            </div>
          </div>
