...
export function ECRDetailView({
  ...
  const stageProgress = ((currentStageIndex + (ecr.status === 'RELEASED' ? 1 : 0)) / STAGE_ORDER.length) * 100;
  ...
  return (
    <div className="p-10 space-y-10 animate-in fade-in duration-1000">
          ...
          {ecr.releasedAt && (
            <>
              ...
              <div className="space-y-1.5">
                ...
                <p className="font-extrabold text-lg text-green-600">{formatDate(ecr.releasedAt)}</p>
              </div>
            </>
          )}
        </div>
            ...
            <Card className="p-12 relative overflow-hidden bg-slate-950 border-none shadow-[0_20px_50px_rgba(0,0,0,0.2)] rounded-[2.5rem] group">
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-12">
                   <h2 className="text-xs font-black text-primary uppercase tracking-[0.3em]">Workflow Progression</h2>
                   <div className="px-4 py-1.5 bg-white/5 rounded-full border border-white/10">
                      <p className="text-[10px] font-bold text-white/50">{Math.round(stageProgress)}% Complete</p>
                   </div>
                </div>
                ...
                <div className="relative pt-4 pb-8">
                  ...
                  <div className="relative flex justify-between items-start">
                    {STAGE_ORDER.map((stage, index) => {
                      ...
                      return (
                        <div key={stage} className="flex flex-col items-center flex-1 z-10 group/node">
                          <div className={cn(
                            "size-10 rounded-full flex items-center justify-center transition-all duration-500 border-4",
                            isPassed && !isSkipped ? "bg-primary border-slate-950 text-white shadow-[0_0_15px_rgba(var(--primary),0.4)]" :
                            isSkipped ? "bg-slate-800 border-slate-900 text-slate-500" :
                            isActive ? "bg-white border-primary text-slate-950 scale-125 shadow-[0_0_30px_rgba(255,255,255,0.2)]" :
                            "bg-slate-900 border-slate-800 text-slate-600"
                          )}>
                            {isPassed && !isSkipped ? <CheckCircle2 className="size-5" /> : <span className="text-[10px] font-black">{index + 1}</span>}
                          </div>
                          ...
                          <div className="mt-6 text-center space-y-1 w-20">
                            ...
                            {isActive && <div className="size-1 bg-primary rounded-full mx-auto animate-ping" />}
                          </div>
                        </div>
                      );
                    })}
                   ...
                   <div className="bg-white m-px p-8 rounded-4xl space-y-8">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-4">
                            ...
                            <div>
                               <h3 className="text-lg font-black tracking-tight text-slate-900">Immediate Action Required</h3>
                               <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Complete the following to advance the workflow</p>
                            </div>
                         </div>
                         <div className="px-3 py-1 bg-primary/5 border border-primary/10 rounded-lg">
                            <span className="text-[10px] font-black text-primary uppercase">{STAGE_SHORT_LABELS[ecr.currentStage as StageType]}</span>
                         </div>
                      </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="p-6 overflow-hidden relative group border-primary/10 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-bold text-muted-foreground uppercase">Project Engineer</p>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-between items-start pt-4 px-4">
                ...
                {STAGE_ORDER.map((stage, index) => {
                  ...
                  const isPassed = index < STAGE_ORDER.indexOf(ecr.currentStage as StageType) || ecr.status === 'RELEASED';
                  const isSkipped = 
                    (stage === 'COSTING' && designInitialForm?.isSkipCosting) ||
                    (stage === 'PROJECT_MANAGER' && designInitialForm?.isSkipProjectManager) ||
                    (stage === 'DESIGN_ENGINEER_MEETING' && designInitialForm?.isSkipMeeting) ||
                    ...
                    <div key={stage} className="flex flex-col items-center flex-1 z-10">
                      ...
                      <div className="mt-5 text-center px-1">
                        ...
                        <div className="mt-1 h-3 flex items-center justify-center">
                          {isActive && (
                            <span className="text-[9px] font-bold text-primary animate-pulse tracking-tight">IN PROGRESS</span>
                          )}
                          {isSkipped && (
                            <span className="text-[9px] font-bold text-zinc-500 italic">SKIPPED</span>
                          )}
                        </div>
                      </div>
                    </div>
      ...
      <Tabs defaultValue="history" className="space-y-6 pt-4" value={activeTab} onValueChange={setActiveTab}>
        <div className="flex items-center justify-between border-b pb-1 overflow-x-auto">
          <TabsList className="bg-transparent h-auto p-0 gap-8">
            <TabsTrigger value="history" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
              <History className="size-4" /> Activity
            </TabsTrigger>
            <TabsTrigger value="design" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
              <FileText className="size-4" /> Design
            </TabsTrigger>
            <TabsTrigger value="costing" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
               <DollarSign className="size-4" /> Costing
            </TabsTrigger>
            <TabsTrigger value="pm" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
              <User className="size-4" /> PM Review
            </TabsTrigger>
            <TabsTrigger value="meeting" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
              <Users className="size-4" /> Meeting
            </TabsTrigger>
            <TabsTrigger value="quality" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none pb-3 px-1 text-sm font-bold flex gap-2">
                 ...
                 {stageHistories.map((log) => (
                   <div key={log.id} className="relative flex gap-10 group">
                     <div className={cn(
                       ...
                       log.flowStatus === 'RETURNED' ? "bg-orange-500 shadow-orange-500/20" : 
                       log.flowStatus === 'PROCEED' ? "bg-green-500 shadow-green-500/20" :
                       "bg-slate-400"
                     )}>
                        {log.flowStatus === 'RETURNED' ? 'RT' : log.flowStatus === 'PROCEED' ? 'OK' : 'SK'}
                     </div>
                     <div className="flex-1 min-w-0">
                       <div className="flex items-center justify-between mb-2">
                         <span className="font-black text-xs uppercase tracking-widest text-primary">
                            ...
                            {log.flowStatus === 'RETURNED' && <span className="ml-3 text-[9px] text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full ring-1 ring-orange-200">RETURNED</span>}
                         </span>
           ...
           <Card className="p-10 border-none shadow-xl overflow-hidden relative">
             ...
             <div className="flex items-center justify-between mb-10 border-b pb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">Stage 1: Primary Details</p>
                </div>
                {designInitialForm?.flowStatus && <FlowStatusBadge status={designInitialForm.flowStatus} />}
             </div>
             ...
             {designInitialForm ? (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-16 relative">
                 <div className="space-y-8">
                    <div className="space-y-4">
                       <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em]">Detailed Change Description</p>
                 ...
                 <div className="space-y-10">
                    <div className="grid grid-cols-2 gap-10">
                      <div className="space-y-3">
                        <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em]">Customer CR Ref</p>
                        <p className="font-mono text-xl font-black text-primary">{designInitialForm.customerCrNumber || 'NONE'}</p>
                      </div>
                      <div className="space-y-3">
                        <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em]">Receipt Date</p>
                        <p className="font-black text-xl">{formatDate(designInitialForm.crReceivedOn)}</p>
                      </div>
                    </div>
                    
                    <div className="p-6 border rounded-2xl bg-slate-50 dark:bg-zinc-900 border-primary/5">
                      <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-6">Workflow Configuration</p>
                      <div className="space-y-4">
                         {[
                           { label: 'Costing Assessment', value: designInitialForm.isSkipCosting },
                           { label: 'Project Management', value: designInitialForm.isSkipProjectManager },
                           ...
                           <div key={item.label} className="flex items-center justify-between group">
                             ...
                             {item.value ? (
                               <span className="text-[9px] font-black bg-orange-100 text-orange-700 px-3 py-1 rounded-full uppercase">Skipped</span>
                             ) : (
                               <span className="text-[9px] font-black bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full uppercase">Mandatory</span>
                             )}
                           </div>
                         ))}
                      </div>
               ...
               <div className="py-24 text-center border-2 border-dashed rounded-[32px] bg-muted/10">
                 ...
                 <p className="text-sm text-muted-foreground/60 max-w-xs mx-auto mt-2">The design initial form has not been submitted for this engineering change request.</p>
               </div>
             )}
           </Card>
        </TabsContent>
           ...
           <Card className="p-10 border-none shadow-xl">
             <div className="flex items-center justify-between mb-10 border-b pb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">Stage 2: Cost Analysis</p>
                </div>
              ...
              {costingForm ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <div className="p-8 bg-slate-50 border rounded-3xl text-center space-y-4">
                      <p className="text-[11px] font-black text-muted-foreground uppercase tracking-wider">Non-Recurring Cost (NRC)</p>
                      <p className="text-4xl font-black text-primary">${costingForm.nrcAmount || '0.00'}</p>
                   </div>
                   <div className="p-8 bg-slate-50 border rounded-3xl text-center space-y-4">
                      <p className="text-[11px] font-black text-muted-foreground uppercase tracking-wider">Recurring Cost (RC)</p>
                      <p className="text-4xl font-black text-primary">${costingForm.rcAmount || '0.00'}</p>
                   </div>
                   <div className="p-8 bg-primary text-primary-foreground rounded-3xl text-center space-y-4 ring-8 ring-primary/5">
                      <p className="text-[11px] font-black opacity-70 uppercase tracking-wider">Total Impact Est.</p>
                      <p className="text-4xl font-black">${(Number(costingForm.nrcAmount || 0) + Number(costingForm.rcAmount || 0)).toFixed(2)}</p>
                   </div>
                </div>
              ) : (
           ...
           <Card className="p-10 border-none shadow-xl">
             <div className="flex items-center justify-between mb-10 border-b pb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">Stage 3: Project Manager Approval</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="space-y-6">
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground uppercase">PO Receipt Date</p>
                         <p className="text-xl font-black">{pmForm.poReceiptDate ? formatDate(pmForm.poReceiptDate) : 'NOT RECEIVED'}</p>
                      </div>
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground uppercase">ROA Reference</p>
           ...
           <Card className="p-10 border-none shadow-xl">
             <div className="flex items-center justify-between mb-10 border-b pb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">Stage 4: Technical Release</p>
                </div>
             ...
             {meetingForm ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground uppercase">Meeting Date</p>
                         <p className="font-black">{meetingForm.meetingDate ? formatDate(meetingForm.meetingDate) : '—'}</p>
                      </div>
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground uppercase">Epicor Release</p>
                         <p className="font-black">{meetingForm.epicorReleaseDate ? formatDate(meetingForm.epicorReleaseDate) : 'PENDING'}</p>
                      </div>
                   </div>
                   <div className="space-y-4">
                      <p className="text-[10px] font-black text-muted-foreground uppercase">Meeting Minutes</p>
           ...
           <Card className="p-10 border-none shadow-xl">
             <div className="flex items-center justify-between mb-10 border-b pb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">Stage 5: Final Inspection</p>
                </div>
                   ...
                   <div className="space-y-8">
                      <div className="flex items-center gap-4">
                         <div className={cn(
                            ...
                            qualityForm.verificationResult === 'PASSED' ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"
                         )}>
                            {qualityForm.verificationResult}
                         </div>
                         <div>
                            <p className="text-[10px] font-black uppercase text-muted-foreground">Verified On</p>
                            <p className="font-black">{qualityForm.verificationDate ? formatDate(qualityForm.verificationDate) : '—'}</p>
                         </div>
                      </div>
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground uppercase">Train Set Verified</p>
                         <p className="text-xl font-black">{qualityForm.verifiedInTrainSet || 'N/A'}</p>
                      </div>
                   </div>
                   <div className="space-y-4">
                      <p className="text-[10px] font-black text-muted-foreground uppercase">Findings & Compliance</p>
                      <div className="p-6 bg-slate-50 border rounded-2xl min-h-[100px] text-sm font-medium">
