...
async function getDashboardStats() {
  try {
    const [statusCounts, projectCounts, totalEcrs, releasedEcrs, onHoldEcrs, recentActivity] =
      await Promise.all([
        prisma.ecr.groupBy({
          by: ['status'],
          _count: { _all: true },
...
export default async function DashboardPage() {
  const { totalEcrs, releasedEcrs, onHoldEcrs, ecrStats, projectStats, recentActivity } =
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-6">
          ...
          <p className="text-3xl font-bold text-blue-600">
            {Number(totalEcrs) - Number(releasedEcrs) - Number(onHoldEcrs)}
          </p>
        </Card>
      </div>
