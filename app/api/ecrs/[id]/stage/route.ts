        ...
        if (existing.length > 0) {
          await sql`
            UPDATE design_meeting_forms SET
              ...
              remark = ${remark || null},
              flow_status = ${meetingFlowStatus}::"ECRFlowStatus",
              processed_on = NOW(),
              submitted_at = NOW(),
              updated_at = NOW()
            WHERE ecr_id = ${id}
          ...
          const [newForm] = await sql`
            ...
            VALUES (${id}, ${formData.meeting_date || null}, ${formData.epicor_release_date || null}, ${formData.ern_release_date || null}, ${formData.meeting_notes || null}, ${isNotApplicable}, ${remark || null}, ${meetingFlowStatus}::"ECRFlowStatus", NOW(), NOW())
            RETURNING id
          `;
